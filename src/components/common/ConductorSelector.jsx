...
// Componente virtualizado para cada elemento de conductor
const VirtualizedConductorItem = React.memo(({ index, style, data }) => {
  const { items, selectedConductor, onSelect } = data;
  const conductor = items[index];
  const isSelected = selectedConductor?.idCon === conductor.idCon;

  return (
    <div style={style}>
      <MenuItem onClick={() => onSelect(conductor)} sx={{ py: 1 }}>
        <Box
          display="flex"
          alignItems="center"
...
const ConductorSelector = React.memo(
  ({
    ...
    // Altura de cada item en la lista virtualizada
    const itemHeight = 70;
    ...
    // Filtrar conductores basado en la búsqueda con optimización
    const filteredConductores = useMemo(() => {
      ...
      return conductores.filter(
        (conductor) =>
          ...
          conductor.empresa?.toLowerCase().includes(searchLower) ||
    ...
    return (
      <Box sx={{ width: "100%", maxWidth: "530px" }}>
        ...
        >
          ...
          <SearchIcon
            sx={{ ml: 1, color: disabled ? "text.disabled" : "text.secondary" }}
          />
        </Button>
          ...
          slotProps={{
            paper: {
              style: {
                ...
                borderRadius: "4px",
                marginTop: "4px",
                overflowY: "hidden", // Dejar que react-window maneje el scroll
              },
            },
          }}
        >
          ...
          {/* Lista virtualizada de conductores */}
          {filteredConductores.length > 0 ? (
            <List
              height={Math.min(
                maxListHeight,
                filteredConductores.length * itemHeight
              )}
              width="100%"
